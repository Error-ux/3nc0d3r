...
import config
from utils.media import get_video_info, get_crop_params, async_generate_thumbnail, get_vmaf, upload_to_cloud
...
async def main():
    ...
    from utils.rename import detect_quality
    if _auto_downscaled:
    ...
    # UI Labels
    # When is_hdr the vf chain tonemaps to SDR — the OUTPUT is always SDR.
    hdr_label      = "SDR (tonemapped)" if is_hdr else "SDR"
    ...
    cmd = [
        ...
        *video_filters,
        ...
        "-pix_fmt", "yuv420p10le",
        "-crf", str(final_crf),
        # Always tag BT.709 SDR color metadata. When is_hdr the vf tonemap chain
        # converts HDR10 → SDR bt709; when not is_hdr the source is already SDR.
        # Skipping these tags causes players/displays to misread the color space.
    ...
    try:
        ...
        if config.RUN_VMAF:
            # Send VMAF progress to a fresh message so it doesn't clobber upload state.
            _vmaf_msg = None
            try:
                _vmaf_msg = await app.send_message(
                    config.CHAT_ID,
                    "<b>[ SYSTEM.ANALYSIS ] Running VMAF + SSIM...</b>",
            ...
            async def vmaf_tg_writer(payload):
                ...
                try:
                    await app.edit_message_text(
                        config.CHAT_ID, _vmaf_msg.id, ui,
                        parse_mode=enums.ParseMode.HTML,
                    )
                except Exception:
                    pass
        ...
        # Convert Pyrogram buttons to (text, url) tuples for Telethon
        buttons_data = []
        ...
        sent_msg_id = await telethon_upload_file(
            ...
            caption=report,
            thumb=thumb,
            buttons_data=buttons_data or None,
