...
async def main():
    ...
    try:
        ...
        if config.RUN_VMAF:
            async def _vmaf_kv_writer(payload):
                await tg_edit(
                    tg_state, tg_ready,
                    get_vmaf_ui(payload["vmaf_percent"], payload["fps"], payload["eta"]),
                )
            vmaf_val, ssim_val = await get_vmaf(config.FILE_NAME, crop_val, width, height, duration, fps_val, kv_writer=_vmaf_kv_writer)
        else:
