...
Outputs : part_00.mkv, part_01.mkv, …  (in --out-dir)
          Prints a JSON array of filenames to stdout.

CLI usage:
    python3 split.py source.mkv [--chunks 10] [--out-dir .]
    python3 split.py source.mkv --print-matrix   # also emit to $GITHUB_OUTPUT
"""
...
def split(
    ...
    print("  Running ffmpeg segment muxer…", flush=True)
    ...
    if not chunks:
        print("[ERROR] No part_*.mkv files found after split.")
        sys.exit(1)

    print(f"✅  Split complete → {len(chunks)} chunks", flush=True)
    for c in chunks:
...
def write_github_output(chunks: list[Path], chunk_count: int) -> None:
    ...
    print(f"  → matrix_json and chunk_count written to $GITHUB_OUTPUT", flush=True)
